  ...
  var [computing, setComputing] = useState("false");
    ...
    function posting(domText, probText, isNewPlan) {
      ...
      if ((supportedContent) == true) {
        ...
        fetch("//18.170.230.213:5001/package/optic/solve", {
          method: "POST",
        ...
        // Query the result every 0.5 seconds while the job is executing
          const queryResult = () => {
          fetch('//18.170.230.213:5001' + solveRequestUrl.result).then(response => response.json()).then(result => {status = result.status || "";
            if (status === 'PENDING') {
